    ...
    private startGame(): void {
        ...
        const getTickMs = () => {
            if (this.state.speedMode === "hard") {
                // Hard mode: starts at baseTickMs, gets faster as snake grows
                // Max speed capped at 40ms
...
export default function (pi: ExtensionAPI) {
    ...
    pi.on("agent_start", async (_event, ctx) => {
        ...
        // We launch the UI but we DON'T await it immediately here 
        // because we want the agent to keep running.
        // However, ctx.ui.custom is blocking for the TUI.
        // To allow the agent to work, we trigger it and store the 'done' resolver.
